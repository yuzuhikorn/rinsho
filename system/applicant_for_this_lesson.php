...
<?php
	...
	connect_to_mysql("select count(状態) from applicant where (状態=3 or 状態=0) and 希望研修ID='".$applicated_lesson_id."'");
	mysql_close($link);
	$row = mysql_fetch_assoc($res);
	print "上限:".$upper_limit."人　";
	print " ";
	$how_many_room=$upper_limit-$row['count(状態)'];
	print "空き:".$how_many_room."人<br>";
	...
	if(@$num = mysql_num_fields($res)){
		...
		while(@$data = mysql_fetch_row($res)){
			...
			if($data[8]==1 ||$data[8]==3){
				print "<div style='visibility:hidden'>";
				print "<span style='width:50px'>";
			}else if($data[8]==2){
				print "<div>";
				print "<span  style='width:50px;visibility:hidden'>";
			}
