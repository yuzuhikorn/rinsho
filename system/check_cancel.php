<?php
	...
	while ($row1 = mysql_fetch_assoc($res1)) {
		...
		//print "研修".$row1['希望研修ID']."<br>";
		connect_to_mysql("select count(状態) from applicant where 状態=3 and 希望研修ID='".$row1['希望研修ID']."'");//対象の研修は何人「入金済み」か
				$res3=$res;
				while ($row3 = mysql_fetch_assoc($res3)) {
				//print "今 ".$row2['count(状態)']."人が状態「0(入金待ち)」";
					//print " ";
					//print "今 ".$row3['count(状態)']."人が状態「3(入金済み)」";
					//print "上限:".$upper_limit."人";
					//print " ";
					$how_many_room=$upper_limit-$row2['count(状態)']-$row3['count(状態)'];
					//print $how_many_room."人の余裕がある<br>";
				}
			if($how_many_room>0){//入金待ちの人数が定員以下だったら
				
				connect_to_mysql("select * from applicant inner join lesson on applicant.希望研修ID=lesson.研修ID where applicant.状態=2 and 希望研修ID='".$row1['希望研修ID']."'");//キャンセル待ちになっている希望者と希望中の研修の情報
				$res4=$res;
				//				if (strtotime(date('Y-m-d H:i:s')) <= strtotime('-3 week',strtotime($row4['start']))) {
				while ($row4 = mysql_fetch_assoc($res4)) {
				$status=0;
				$applicant_id=$row4['希望者ID'];
				$lesson_id=$row4['希望研修ID'];
